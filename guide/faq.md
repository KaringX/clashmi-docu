...
## X-HWID 是什么,有什么作用 {#x-hwid}
::: tip
- X-HWID: 可以理解为硬件标识符,某些VPN服务商需要在你下载/更新订阅的时候,需要此信息才会下发配置文件,需要你手动开启才会将此信息提供给你的VPN服务商
- 注意:不同于其他实现,本App里的X-HWID是使用和硬件无法的算法自动生成,并且某些系统上,删除本应用后,此信息也会一并清除,重新安装将会重新生成一个新的
...
::: tip
 - 检查 核心设置-TUN-'附加HTTP代理到VPN'
 - 关闭 '附加HTTP代理到VPN' 后重新连接
